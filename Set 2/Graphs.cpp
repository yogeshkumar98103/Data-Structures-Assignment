...
class Base_Graph<data_t, GraphRepresentation_AdjacencyList, edge_t, weight_t>{
  ...
  public:
    ...
    pair<vector<int>, vector<int>> shortestPath(int source){
    ...
    template <typename U = weight_t, typename enable_if<is_same<U, void>::value>::type * = nullptr>
    pair<int, vector<int>> shortestPath(int source, int destination){
    ...
    weight_t kruskal(){
        DisjointSet ds(this->vertex_count + 1);
        for(int i = 1; i <= vertex_count; ++i){
            ds.make_set(i);
        }

        vector<Edge<weight_t>> edges;
        for(int i = 0; i < this->vertex_count; i++){
            for(const edge_t& edge: this->adj_list[i]){
                edges.push_back({i, edge.to, edge.weight});
            }
        }

        sort(edges.begin(), edges.end(), [](const Edge<weight_t>& e1, const Edge<weight_t>& e2)->bool{
            return e1.weight >= e2.weight;
        });

        weight_t ans = 1;
        for(const Edge<weight_t>& e: edges){
